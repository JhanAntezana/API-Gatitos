const api = axios.create({
    baseURL: 'https://api.thecatapi.com/v1',//se establece la parte del url del api que se usa en todo, dependiendo de cada uso se completara el URL
})
api.instance.defaults.headers.common['X-API-KEY'] = '8a2b70f9-9790-4ca9-a19c-6272395402b0';//envia de manera automática el api-key a todas las peticiones.
...
async function addToFavorites(id){
    ...
    //CON AXIOS
    const response = await api.post('/favorites',{//en ves de response tambien se puede colocar {data, status} para uso directo de estas variables
        image_id: id
    })
    if(response.status !== 200){
        spanError.innerHTML = 'Hubo un error '+response.status + response.data.mesange;
    }else{
       reloadFavoriteCats();
    }
}
